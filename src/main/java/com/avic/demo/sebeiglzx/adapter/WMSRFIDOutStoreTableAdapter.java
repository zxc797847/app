...
public class WMSRFIDOutStoreTableAdapter extends BaseAdapter {
	private List<InitProRfidFkbBean> list;
	private LayoutInflater inflater;

	public WMSRFIDOutStoreTableAdapter(Context context, List<InitProRfidFkbBean> list) {
		this.list = list;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(R.layout.list_wms_out_store_item, null);
			viewHolder.goodId = (TextView) convertView.findViewById(R.id.text_id);
			viewHolder.tv_CPMC = (TextView) convertView.findViewById(R.id.tv_CPMC);
		...
		if (proRfidFkbBean.getIsCheck()) {
			convertView.setBackgroundColor(Color.parseColor("#00BF5F"));
		} else {
			convertView.setBackgroundColor(Color.WHITE);
		}

		viewHolder.ck_XZ.setOnClickListener(new View.OnClickListener() {
			@Override
